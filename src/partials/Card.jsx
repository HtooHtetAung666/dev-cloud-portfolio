...
import profile from '../images/htoohtet.JPG'
...
const Card = ({ name, title, social: { github, email, linkedin } }) => {
    return (
        <div className='relative flex flex-col justify-center max-w-xs mx-auto bg-white rounded-xl p-5'>
            <div>
                <img
                    className='w-32 h-32 md:w-36 md:h-36 object-fill mx-auto shadow-xl rounded-full'
                    src={profile}
                    alt="face"
                />
            </div>
            <div className='text-center mt-5'>
                <p className='text-xl sm:text-2xl font-semibold text-[#253D63]'>{name}</p>
